...
PoseHistory::GetBestPoseMatch(const vr::HmdMatrix34_t& pose) const {
    ...
    // The buffer holds about 1.7 seconds of tracking, so sweeping the head back over
    // an orientation it recently held leaves several entries matching equally well.
    // Searching oldest first returned the oldest of those. Search newest first so a
    // tie resolves to the most recent entry, and stop at the window bound.
